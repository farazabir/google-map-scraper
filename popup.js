document.addEventListener('DOMContentLoaded', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        ...
        var resultsCount = document.getElementById('resultsCount'); // Get count element
        ...
        actionButton.addEventListener('click', function() {
            ...
            document.getElementById('message').textContent = "Scraping data...";
            ...
            chrome.scripting.executeScript({
                ...
                var dataCount = 0; // Initialize count
                ...
                if (results && results[0] && results[0].result) {
                    results[0].result.forEach(function(item) {
                        ...
                        dataCount++; // Increment count for each row added
                    });
                }

                // Update the count display
                updateResultsCount(dataCount);

                // Update status and enable download if we have results
                if (dataCount > 0) {
                    downloadCsvButton.disabled = false;
                    document.getElementById('message').textContent = `Scraping completed! Found ${dataCount} entries.`;
                } else {
                    document.getElementById('message').textContent = "No data found. Try scrolling down to load more results.";
                }
        ...
        // Function to update the results count display
        function updateResultsCount(count) {
            if (count === 0) {
                resultsCount.textContent = "0 entries";
            } else if (count === 1) {
                resultsCount.textContent = "1 entry";
            } else {
